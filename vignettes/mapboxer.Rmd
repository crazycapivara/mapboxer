...
motor_vehicle_collisions_nyc %>%
  ...
  add_circle_layer(
    ...
    popup = "Number of injured persons: {{injured}}"
  )
```
...
background color as basemap. Therefore, you can use the helpers `basemap_raster_style()` or `basemap_background_style()`:
...
If all elements are of the same type you can also use a vector, e. g. `c("get", "<data-property>")`.
