...
This guide covers the basic usage of the components that are needed to create a visualization:
...
* [Shiny Bindings](https://shiny.rstudio.com/): With `renderMapboxer()` and `mapboxerOutput()` you can integrate your visualizations in Shiny apps. Furthermore, you can use `mapboxer_proxy()` and `send_mapboxer_update()` to update the already rendered widget.
...
quakes %>%
  ...
  # Add a layer styling the data of the source
  add_circle_layer(
    circle_color = "red",
    circle_radius = 5,
...
With `mapboxer()` you create a map object. This is the main component of your vizualization. To add components like layers or controls or to modify your map you use the `add_*` and `set_*` functions. The parameters passed to `mapboxer()` itself are used to configure the initial setup of your map. The `style` parameter sets the style of the basemap. By default mapboxer uses a [Carto vector style](https://github.com/CartoDB/basemap-styles). It is also possible to use raster tiles or a
...
quakes_source_from_df <- quakes %>%
  as_mapbox_source(lat = "lat", lng = "long")
```
...
See the [Sources API Reference](https://docs.mapbox.com/mapbox-gl-js/style-spec/sources/) for available options for the used source type. Sources are either passed to the `add_*_layer` functions or as first parameter to `mapboxer()`.
...
quakes_map %>%
  add_circle_layer(
    circle_color = list("get", "color"),
    # Expression to display only data where 'mag > 5'
    filter = list(">", "mag", 5)
  )
```
...
With `mapboxer_proxy()` and `send_mapboxer_update()` you can update your already rendered map:
